...
import "./stye.scss"; // CSS faylini import qiling
import { ASSETS } from "../../assets/images/assets";
import { modalsStore } from "../../store";

const Notification = ({ message, onClose, stateValue }: any) => {
  ...
  useEffect(() => {
    if (!visible && initialDelay) {
      const hideTimer = setTimeout(() => {
        closeModal("cashModal")// Animatsiya tugagandan so'ng xabarni tozalash
  ...
  return (
    <div className={`notification ${visible ? "slide-down" : "slide-up"}`}>
      <div className="w-[90%] bg-white  slide-down-item  px-[20px] gap-x-[15px] py-[10px] rounded-[40px] mx-auto flex items-center">
        <img src={ASSETS.NotificationImg} alt="" />
